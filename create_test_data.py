...
def generate_segment(
        ...
        doc_count_max: int = 10, sentence_min: int = 20, labeled: bool = False, embeddings: bool = False) -> tuple: 
    '''
    ...
    Args:
        ...
        doc_count_min: minimum number of documents to pull from. Inclusive
        doc_count_max: maximum number of documents to pull from. Exclusive
        sentence_min: minimum number of sentences per document selected
        labeled: has labeled topic codings
        embeddings: has sentence embeddings
    
    Returns:
        segment (list): List of sentences
        embedding (list): List of embedding for sentences
    ...
    if labeled and not ('topic_counts' in t.columns):
        raise Exception("entered labeled=True but 'topic_counts' column does not exist in df")
    if embeddings and not ('embedding' in t.columns):
        raise Exception("entered embeddings=True but 'embedding' column does not exist in df")
    ...
    #if embeddings are included    
    if embeddings and 'embedding' in text.columns:
...
def transcript_pickle_to_df(path="data/transcripts.pickle"):
    '''
